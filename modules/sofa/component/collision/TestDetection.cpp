...
{	
	...
	if (cm->empty())
		return;
	for (sofa::helper::vector<CollisionModel*>::iterator it = collisionModels.begin(); it != collisionModels.end(); ++it)
	{
					...
					if ( child1Offirst.valid() )
					{
						if (child1Ofsecond.valid() )
						{
							...
							else
							{
								for (CollisionElementIterator it1 = child1Offirst; it1 != child2Offirst; ++it1) 
									for (CollisionElementIterator it2 = child1Ofsecond; it2 != child2Ofsecond; ++it2) 
										finalIntersector->intersect( it2, it1, outputs );
							}
						}
						else{
							...
							{
								for (CollisionElementIterator it1 = child1Offirst; it1 != child2Offirst; ++it1) 
									finalIntersector->intersect( it1, pair.second, outputs );
							}
							else
							{
								for (CollisionElementIterator it1 = child1Offirst; it1 != child2Offirst; ++it1) 
									finalIntersector->intersect( pair.second, it1, outputs );
							}
						}
					}
					else if (child1Ofsecond.valid() )
					{
						...
						if (child1Ofsecond.getCollisionModel()==finalcm1)
						{
							...
							for (CollisionElementIterator it1 = child1Ofsecond; it1 != child2Ofsecond; ++it1)
								finalIntersector->intersect( it1, pair.first, outputs );
						}
						else
						{
							//cout << " introduce the children of pair.second only " << endl;
							for (CollisionElementIterator it1 = child1Ofsecond; it1 != child2Ofsecond; ++it1)
								finalIntersector->intersect( pair.first, it1, outputs );
						}
					}
					else
					{
						if (pair.first.getCollisionModel()->getFirst()==cm1)
							intersector->intersect( pair.first,pair.second, outputs);
						else
							intersector->intersect( pair.second,pair.first, outputs);
					}
