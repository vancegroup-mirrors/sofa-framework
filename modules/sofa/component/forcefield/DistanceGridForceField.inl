...
void DistanceGridForceField<DataTypes>::init()
{
    ...
    {
	core::componentmodel::topology::BaseMeshTopology* topology = this->getContext()->getMeshTopology();
	if (topology && topology->getNbTriangles() > 0)
	{
	    const core::componentmodel::topology::BaseMeshTopology::SeqTriangles& triangles = topology->getTriangles();
	    Real sumArea = 0;
	    Real sumSArea = 0;
...
template<class DataTypes>
void DistanceGridForceField<DataTypes>::drawDistanceGrid(float size)
{
    if (!grid) return;
	...
	simulation::getSimulation()->DrawUtility.drawLines(pointsLineIn, 1, defaulttype::Vec<4,float>(1,0,0,1));
    if (!pointsLineOut.empty())
	simulation::getSimulation()->DrawUtility.drawLines(pointsLineOut, 1, defaulttype::Vec<4,float>(1,0,1,1));
    const sofa::helper::vector<TContact>& tcontacts = this->tcontacts.getValue();
    if (!tcontacts.empty())
    {
