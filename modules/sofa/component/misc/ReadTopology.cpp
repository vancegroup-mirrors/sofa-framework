...
******************************************************************************/
#include <sofa/component/misc/ReadTopology.inl>
...
void ReadTopologyCreator::addReadTopology(core::componentmodel::topology::BaseMeshTopology* topology, simulation::Node* gnode)
{
	...
	if (  rt == NULL )
	{
	    rt = new sofa::component::misc::ReadTopology(); gnode->addObject(rt);
        ...
        rt->f_filename.setValue(ofilename.str());  rt->f_listening.setValue(false); //Desactivated only called by extern functions
        if (init) rt->init();
