...
public:
	typedef Vec3d Vec3;
        ...
        std::string filename; ///< Mesh used to initialize filled portion of the grid

	FittedRegularGridTopology();
	
	FittedRegularGridTopology(int nx, int ny, int nz);

        void setSize(int nx, int ny, int nz) { RegularGridTopology::setSize(nx, ny, nz); }
...
protected:
        ...
        virtual void addTriangle(const Vec3& p1, const Vec3& p2, const Vec3& p3);

        virtual void addTriangle(const Vec3& p1, int c1, const Vec3& p2, int c2, const Vec3& p3, int c3, int depth=0);

	/// active flag for each cube (true if the given cube contains or is surrounded by mapped points).
	sofa::helper::vector<bool> activeCubeFlag;
