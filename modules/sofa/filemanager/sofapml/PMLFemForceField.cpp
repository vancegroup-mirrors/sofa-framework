...
Vec3d PMLFemForceField::getDOF(unsigned int index)
{
...
{
	forcefield = new TetrahedronFEMForceField<Vec3dTypes>;
	//if(poisson==0.0)poisson=0.49;
	//if(young==0.0) young=5000.0;

	((TetrahedronFEMForceField<Vec3dTypes>*)forcefield)->setPoissonRatio(poisson);

	((TetrahedronFEMForceField<Vec3dTypes>*)forcefield)->setYoungModulus(young);

	if(deformationType== "SMALL")
		((TetrahedronFEMForceField<Vec3dTypes>*)forcefield)->setMethod(0);
	if(deformationType== "LARGE") 
		((TetrahedronFEMForceField<Vec3dTypes>*)forcefield)->setMethod(1);
	if(deformationType== "POLAR") 
		((TetrahedronFEMForceField<Vec3dTypes>*)forcefield)->setMethod(2);
