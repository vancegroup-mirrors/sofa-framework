...
inline Real SPHFluidCalcDensity(Real4 x1, Real4 x2, Real density, GPUSPHFluid* params)
{
	Real4 n = x2-x1;
//	Real d2;
	Real d2 = native_sqrt(n.x*n.x+n.y*n.y+n.z*n.z);
	if (d2 < params->h2)
	{
	...
	for (int cell = get_group_id(0); cell < size; cell += get_num_groups(0))
	{
		...
		for (int px0 = range_x; px0 < ghost; px0 += BSIZE)
		{
			...
			for (int py0 = range_x; py0 < range_y; py0 += BSIZE)
			{
				...
				{
					int index2 = cells[py];
					Real4 xj = (Real4)(x[3*index2],x[3*index2+1],x[3*index2+1],0);
					temp_x[tx3  ] = xj.x;
					temp_x[tx3+1] = xj.y;
					temp_x[tx3+2] = xj.z;
				}
	...
	for (int cell = get_group_id(0); cell < size; cell += get_num_groups(0))
	{
		...
		for (int px0 = range_x; px0 < ghost; px0 += BSIZE)
		{
			...
			{ // actual particle -> write computed force
				f[index*3]   += force.x/100;
				f[index*3+1] += force.y/100;
				f[index*3+2] += force.z/100;
			}
		}
	}
}
