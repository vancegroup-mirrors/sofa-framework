...
void Automate::load(char *_automateType, bool _periphInSimulation)
{
	if (strcmp(_automateType, "simpleMonoFrequency") == 0)
	{
		...
		collR->setVisitor(collisResponseCode);
		...
		collD->setVisitor(collisDetectionCode);
		...
		coll->setVisitor(collisResetCode);
		...
		monoFreqSubAuto->setVisitorList(tmp);
	...
	else if (strcmp(_automateType, "test") == 0)
	{
		...
		incra->setVisitor(incrA);

		NodeExec *reza = new NodeExec("a=0", 1, 0);
		reza->setVisitor(rezA);

		CondExec *condA	= new CondExec();
		condA->setVisitor(testA);
...
Node* Automate::exitCompteur(void)
{
	...
	dec->setVisitor( decompteur );
	test->setVisitor( isFini );
	ex->setVisitor( exitCall );
