...
#include "Sofa-old/Components/DiagonalMass.h"
#include "Sofa-old/Components/TriangleFEMForceField.h"
...
namespace procedural
{
    ...
    GNode* buildRectangularMesh( unsigned nbx, unsigned nby, double sx, double sy, double young, double poisson, double massPerSquareMeter )
    {
        GNode* node = new GNode;

        MechanicalObject<MyTypes>* particles = new MechanicalObject<MyTypes>;
        node->addObject(particles);
        particles->resize(nbx*nby);
        particles->setName("trian_dof");
    

        DiagonalMass<MyTypes,double>* mass = new DiagonalMass<MyTypes,double>(particles);
        node->addObject(mass);
        mass->resize(nbx*nby);
    //mass->setMass( massPerSquareMeter*sx*sy/(nbx*nby) );
...
/*    TriangleFEM3ForceField<MyTypes>* spring = new TriangleFEM3ForceField<MyTypes>(particles);
        node->addObject(spring);
        spring->setYoungModulus( young );
        spring->setPoissonRatio( poisson );*/
    
        TriangleFEMForceField<MyTypes>* spring = new TriangleFEMForceField<MyTypes>(particles);
        node->addObject(spring);
        spring->f_young.setValue( young*10 );
        spring->f_poisson.setValue( poisson );
    
    
    //MyTypes::VecCoord& x = *particles->getX();
        MyTypes::VecCoord& x = *particles->f_X->beginEdit();
        MyTypes::VecDeriv& v = *particles->getV();
        DiagonalMass<MyTypes,double>::VecMass& masses = *mass->f_mass.beginEdit(); 
    
        double xstep = (double) sx/(nbx-1);
        double ystep = (double) sy/(nby-1);
    
        for( unsigned i=0; i<nbx; i++ )
        {
            constraint->addConstraint(i);
            for( unsigned j=0; j<nby; j++ )
            {
                x[i+j*nbx]=Vec3( i*xstep, j*ystep, 0 );
                v[i+j*nbx]=Vec3( 0, 0, 0 );
                masses[i+j*nbx] = massPerSquareMeter*sx*sy/(nbx*nby);
            // divide the mass at the borders and corners
                if( i==0 || i==nbx-1 )
                    masses[i+j*nbx] /= 2;
                if( j==0 || j==nby-1 )
                    masses[i+j*nbx] /= 2;
        ...
        for( unsigned i=1; i<nbx; i++ )
        {
            for( unsigned j=1; j<nby; j++ )
            {
                topology->addTriangle( (i-1)+(j-1)*nbx, (i  )+(j-1)*nbx, (i  )+(j  )*nbx );
                topology->addTriangle( (i  )+(j  )*nbx, (i-1)+(j  )*nbx, (i-1)+(j-1)*nbx );
            }
        }
...
ParticleInertia::ParticleInertia( int n )
{
    ...
    getRoot()->addObject( solver );
    Gravity* gravity = new Gravity;
    gravity->f_gravity.setValue(Vec3(0,0,0) ) ; 
    getRoot()->addObject( gravity );
...
ArticulatedSolid::ArticulatedSolid( int n, int attachType, const Vec3& g )
{
    EulerSolver* solver = new EulerSolver;
    solver->f_printLog.setValue( false );
    getRoot()->addObject( solver );
