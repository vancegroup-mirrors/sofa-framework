...
"""
Name: 'Importer fichier WaveFront (.obj)'
Blender: 242
Group: 'Import'
Tooltip: 'Importe un fichier obj dans la scene Blender'
"""

__author__= "Campbell Barton", "Jiri Hnidek", "modif Vincent Vansuyt (VV)"
__url__= ["blender.org", "blenderartists.org", "http://www.vincentvansuyt.com/bld_Index.htm"]
__version__= "original : 2.0, modif VV : 2.04"
...
# Modifs VV
#
# v2.04 : Passage de la variable SPLIT_MATERIALS, à False pour pouvoir 
#       :   utiliser plusieurs matériaux sur un même maillage
#
# v2_03 : suppression des "print"
#       : passage en codage utf-8 suite à un problème d'accents
#       : renvoie la liste des objets crées suite a l'appel du chargement du fichier obj pour
#       :  que la fonction "load_obj" puisse être utilisée à partir d'autres programmes
#
# v2_02 : première version a partir de l'original 2.46 Blender
#       : mémorisation du chemin d'ouverture
#       : pas de redimensionnement des maillages
...
def load_obj(filepath, CLAMP_SIZE= 1.0, #modif VV (old value = 0.0)\
				CREATE_FGONS= True, \
				CREATE_SMOOTH_GROUPS= True, \
				CREATE_EDGES= True, \
				SPLIT_OBJECTS= True, \
				SPLIT_GROUPS= True, \
				SPLIT_MATERIALS= False, #modif VV (old value = True)\
				IMAGE_SEARCH=True):
	'''
	...
	#print '\tpassing obj file "%s"...' % filepath, VV
	time_sub= sys.time()
	...
	for line in file: #.xreadlines():
		...
		if line.startswith('v '):
			line_split= line.split()
 			# rotate X90: (x,-z,y) NON : pas de rotation autour de X (ligne ci-dessous) ! modif VV
	...
	file.close()
	time_new= sys.time()
	# print '%.4f sec' % (time_new-time_sub) VV
	time_sub= time_new
	
	
	#print '\tloading materials and images...', VV
	...
	#print '\tbuilding geometry...\n\tverts:%i faces:%i materials: %i smoothgroups:%i ...' % ( len(verts_loc), len(faces), len(unique_materials), len(unique_smooth_groups) ), VV
...
def getFichier_memo_dernier_dossier_pour_lire_ou_ecrire_obj():
	"""Renvoie le nom du fichier ou on stocke les dossiers des derniers fichiers obj lus ou enregistres"""
	strNomFichierMemo = Blender.Get('uscriptsdir')
	if not strNomFichierMemo.endswith( os.sep ):
		strNomFichierMemo += os.sep 
	strNomFichierMemo += "dir_last_obj.txt"
	return strNomFichierMemo
...
def load_obj_ui(filepath, BATCH_LOAD= False):
	...
	# Lecture du chemin du fichier importé
	strNomFichierMemo = getFichier_memo_dernier_dossier_pour_lire_ou_ecrire_obj()
	output = open( strNomFichierMemo, 'w' )
	output.write( os.path.dirname( filepath ))
	output.close()
	
	# Debut du script livre avec Blender
	CREATE_SMOOTH_GROUPS= Draw.Create(0)
	CREATE_FGONS= Draw.Create(0)
	CREATE_EDGES= Draw.Create(1)
	SPLIT_OBJECTS= Draw.Create(1)
	SPLIT_GROUPS= Draw.Create(1)
	SPLIT_MATERIALS= Draw.Create(0)# modif VV (ancien = 1)
	MORPH_TARGET= Draw.Create(0)
	CLAMP_SIZE= Draw.Create(1.0)# modif VV (ancien = 10)
...
if __name__=='__main__' and not DEBUG:
	if os and Window.GetKeyQualifiers() & Window.Qual.SHIFT:
		...
		print "----------- debut export obj -----------"
		# Tentative de recuperation du dernier chemin utilise pour un fichier obj
		strNomFichierMemo = getFichier_memo_dernier_dossier_pour_lire_ou_ecrire_obj()
		strCurDir = os.getcwd()
		print "Chemin courant \"%s\""%(strCurDir)
		if os.path.exists( strNomFichierMemo ):
			print "Le fichier memo existe\n"
			input = open( strNomFichierMemo, 'r' )
			strLastDir = input.readline()
			print "Dernier repertoire utilise \"%s\"\n"%( strLastDir )
			input.close()
			if os.path.exists( strLastDir ):
				print "changement vers le dossier \"%s\""%( strLastDir )
				os.chdir( strLastDir ) 
		# Appel du dialogue de choix de fichier
		Window.FileSelector(load_obj_ui, 'Import a Wavefront OBJ', '*.obj')
		# Retablissement du chemin courant
		if strCurDir != os.getcwd():
			os.chdir( strCurDir )
